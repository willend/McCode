\subsection*{A general Monitor for 0D/1D/2D records}
\label{s:monitornd}
...
\subsubsection{Monitoring user variables}
\label{s:monnd:user}
...
\paragraph{Directly setting the user variables (simple)}
...
\paragraph{Indirectly setting the user variables (only for professionals)}
...
\paragraph{Example: Number of neutron bounces in a guide}
...
\subsubsection{Monitoring neutron parameter correlations, PreMonitor\_nD}
