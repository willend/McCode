\subsection*{An incoherent scatterer, the V-sample}
\label{s:v_sample}
...
The component \textbf{Incoherent}
...
The sample dimensions are: outer radius \textit{radius}, height \textit{yheight},
and (for a hollow cylinder) wall \textit{thickness}, see figure \ref{f:v-sample}.
\begin{figure}
  \begin{center}
    \psfrag{ri}{$r_i$}
    \psfrag{ro}{$r_o$}
...
by specifying the width, \textit{xwidth}, the height, \textit{yheight}, and the depth, \textit{zdepth}.

The incoherent and absorption cross sections for V are default
for the component. For other choices, the
parameters \textit{sigma\_inc}, \textit{sigma\_abs},
and the unit cell volume \textit{Vc} should be specified.
For a loosely packed sample, also the packing factor, \textit{pack}
...
a target sphere of radius \textit{focus\_r}, at the position
(\textit{target\_x},\textit{target\_y},\textit{target\_z})
...
\textit{focus\_xw}, \textit{focus\_yh}, or uniformly in angular space
(in a small-angle approximation),
using \textit{focus\_aw}, \textit{focus\_ah}.
