...
We first consider incoherent scattering. The general-purpose
\textbf{Incoherent} component performs incoherent scattering and absorption
for an arbitrary shape; the older, simpler \textbf{V\_sample} component that
previously served this role has moved to the \textbf{obsolete} component
category (see the Component Manual's Obsolete chapter) and is superseded by
\textbf{Incoherent}.
...
According to (\ref{e:probrule}), the neutron weight factor
...
\subsection{Small-angle scattering (SANS) and other sample models}
\index{SANS}

Beyond the simple isotropic small-angle model \textbf{Sans\_spheres}
(scattering from a dilute collection of hard spheres), \MCS provides two
further, much richer routes to SANS sample modelling:
\begin{itemize}
\item native McStas SANS models contributed in the \texttt{contrib}
  category (\texttt{SANSSpheres}, \texttt{SANSCylinders},
  \texttt{SANSNanodiscs}, \texttt{SANSPDB}, and many others -- see the
  Component Manual's Contributed Components chapter), several with
  polydisperse and multiple-scattering variants;
\item the \texttt{sasmodels} category (see the Component Manual's dedicated
  chapter), exposing the same, actively maintained SANS/USANS form-factor
  library used for data fitting in SasView, via the generic
  \texttt{SasView\_model} interface.
\end{itemize}
In general, all samples are assumed to be homogeneous. There is potential in
developing inhomogeneous samples, e.g. with a spatially varying lattice
constant (relevant for stress/strain scanners) or inhomogeneous absorption
(relevant for tomography), and reflectometry sample models beyond
\texttt{Multilayer\_Sample} (contrib). Polarization effects are handled by
a dedicated set of polarisation-aware optics and field components rather
than the samples described in this chapter -- see the Polarisation
chapter, \ref{c:polarization}.
...
\newpage
\section{Samples for resolution-function calculations}
\input{samples/Res_sample}

\input{samples/TOFRes_sample}
