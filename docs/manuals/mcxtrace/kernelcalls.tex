...
By default \MCX uses a 64-bit KISS (``Keep It Simple Stupid'') algorithm for generating pseudo random numbers, which runs on both CPU and GPU/OpenACC. The classic Mersenne Twister\cite{matsumoto1998mersenne} algorithm remains available as a CPU-only, build-time alternative (see chapter~\ref{s:random}).
\begin{itemize}
