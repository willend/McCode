...
(\texttt{mxgui}).}
\item{Next, the \MCX compiler \texttt{mcxtrace} is invoked to translate the instrument and component files into a C program. The program \texttt{mcxtrace} itself is written in C, using the parser \textit{flex} and the compiler compiler \textit{bison}.}
...
\item{The output data may be analyzed and visualized in the same way as regular
    ...
    As of \MCX 3.x, these front-ends (\texttt{mxplot}, \texttt{mxdisplay}, \ldots) are all implemented in Python, using
...
\verb+Template_1Slit_Diff.instr+ file supplied in the
\verb+examples/Templates+ directory of the \MCX distribution.
...
``X-ray Site'' menu) and open the template/example \verb+Template_1Slit_Diff+.
...
variable.}
\end{itemize}
\indexEV{MCXTRACE\_FORMAT}

Next, select ``Run simulation'' from the ``Simulation'' menu.
The \MCX compiler \texttt{mcxtrace} will translate the definition into an executable program.
...
section~\ref{s:mxplot}.\indexMCTOOL{mxplot}{} For other options, execute
...
For a slightly longer gentle introduction to \MCX, see the \MCX tutorial
(available from~\cite{mcxtrace_webpage}), and as of version \version\ built into
the \verb+mxgui+ help menu. For more technical details, read on from
...
\indexMCSW{mcxtrace}{compiler}{(textbf}
This section describes how to run the \MCX compiler \texttt{mcxtrace}\ manually. Often,
...
\begin{bash}
    mcxtrace name.instr
\end{bash}
the compiler \texttt{mcxtrace} will read the instrument definition \verb+name.instr+,
...
By default, the code generated by \texttt{mcxtrace} is ISO-C with
...
macros in the C compiler, without the need to re-run \texttt{mcxtrace}.
...
From \MCX~3.5 onwards an alternative code generator,
\texttt{mcxtrace-antlr}, is available alongside the classic generator.
...
At the time of the \MCX~3.6 release, \texttt{mcxtrace-antlr} is close to
...
The \MCX compiler \texttt{mcxtrace}
...
Usually, the user will not need to worry about this as \texttt{mcxtrace} will
automatically find the required files. But if users build their own component
library in a separate directory or if \texttt{mcxtrace} is installed in an unusual way, it
...
when \texttt{mcxtrace} is compiled.
It defaults to \nolinkurl{/usr/share/mcxtrace/version} on Debian and derivatives,
to \nolinkurl{/usr/local/mcxtrace/version} on RedHat and derivatives
and on other Unix-like systems, including Mac OS~X,
where it is a link to the actual location
\nolinkurl{/Applications/McXtrace-version.app/Contents/Resources/mcxtrace/version},
and \nolinkurl{C:\\mcxtrace-version\\lib} on Windows systems,
...
To make \texttt{mcxtrace} search additional directories for component definitions
and include files, use the \verb+-I+ switch:
\begin{lstlisting}
    mcxtrace -I/home/joe/components -I/home/joe/mcxtrace/include name.instr
\end{lstlisting}
...
By default, \texttt{mcxtrace} will generate a stand-alone C program, which is what is needed
...
For such usage, \texttt{mcxtrace} provides the following options:
\begin{itemize}
\item \verb+--no-main+ This option makes \texttt{mcxtrace} omit the \verb+main()+ function
...
\item \verb+--no-runtime+ Normally, the
  ...
  option makes \texttt{mcxtrace} omit the run-time code from the generated
...
\indexMCSW{mcxtrace}{compiler}{)|)}

%-------------------------------------------------------------------------------
\subsection{Running the C compiler}
\label{s:compile}

After the source code for the simulation program has been generated with \texttt{mcxtrace},
...
\item[\textbf{A}]{\verb+gcc+ which is a very portable, open source, ISO-C compatible
    ...
    your distribution, for Windows the \MCX distribution package includes a
...
which can make quite a difference in speed of \texttt{mcxtrace}-generated simulations. The
...
be available on the \MCX WWW home page~\cite{mcxtrace_webpage}):
\begin{itemize}
...
generate \emph{incorrect code} in some compiler versions. \texttt{mcxtrace} actually puts an
...
Once the simulation program has been generated by the \MCX compiler \texttt{mcxtrace}
...
\begin{lstlisting}
    ./Template_1Slit_Diff.out SLITW=5e-6
\end{lstlisting}
\index{Parameters!instruments}
...
instrument name and a time stamp. The default file name is \verb+mcxtrace+
...
recommended way to exchange x-ray events between \MCX/McXtrace and other
...
(\texttt{mccodelib.mxplotloader}), see section~\ref{s:mxplot}.
\indexMCTOOL{mxplot}{}
...
generated automatically from the \texttt{mxrun} Python source
(\texttt{tools/Python/mxrun/mxrun.py}) by the script
\texttt{gen\_mxrun\_options\_table.py} in this directory, so that they cannot
silently go out of date; run \verb+mxrun --help+ for the same information
at the command line.
...
\item Divide simulation into parts using a file for saving or generating x-ray
  events. In this way, a guide may be simulated only once, saving the x-ray
...
\item Use source optimizers like the components Source\_adapt or
  ...
  x-ray importance sampling can be achieved, or may even sometimes alter the
...
One of the authors of \MCX has developed a very flexible and general data
...
iFit contains wrapper functionality for compiling and running \MCX simulations
...
Our experience is that iFit together with \MCX can be a more robust optimization
solution than the \MCX built-in \texttt{--optimize} mode described next, in
...
Example:
\begin{lstlisting}
mxrun MyBeamline.instr --optimize --optimize-monitor=detector Slit_width=0.1,2
\end{lstlisting}
...
\item inputs the simulation parameters, which are usually numerical values such
  as a mirror curvature or a slit width, taken from an instrument in the
  \verb+examples+ directory of the package.
...
For instance, we could write a function for Python using
\texttt{scipy.optimize} (or Matlab, if preferred) in order to study the
effect of a beamline parameter \texttt{P} on a given monitor's intensity:
\begin{lstlisting}
import subprocess
...
def instr_value(P):
    outdir = f"opt_{P:.3f}"
    subprocess.run(["mxrun", "MyBeamline.instr", "-d", outdir, "-n", "1e6",
                     f"P={P}"], check=True)
    # read back the relevant monitor's plain-text McCode data file directly
    data = np.loadtxt(f"{outdir}/detector.dat")
    return -data[:, 1].sum()   # negative intensity column sum: we minimize below

result = minimize_scalar(instr_value, bounds=(0.1, 2.0), method="bounded")
\end{lstlisting}
will determine the best value of \texttt{P} in order to maximize the mean
detector counts. This is essentially what \texttt{mxrun --optimize} already
...
\begin{description}\indexMCTOOL{mxplot}{}\index{Data formats}
...
instruments as found in the \MCX library, sorted by category/facility (e.g.
\texttt{Templates}, \texttt{ILL}, \texttt{ISIS}, \texttt{ESS},
\texttt{Union\_demos}, \ldots). When
...
\item[Photon count] sets the number of x-rays to
...
\texttt{\textit{sim}.instr} or the simulation program \texttt{\textit{sim}.out} generated by \texttt{mcxtrace},
...
\item \verb+mxdisplay-cad+: exports the instrument geometry to a CAD-compatible
  format.
\item \verb+mxdisplay-mantid_xml+: exports an approximate Mantid Instrument
  Definition File (IDF) describing the detector geometry.
\end{itemize}
For instance, calling
\begin{bash}
mxdisplay-webgl ./Template_1Slit_Diff.out SLITW=5e-6
\end{bash}
...
\indexMCTOOL{mxplotdiff}{textbf}
\indexMCTOOL{mxcoplot}{textbf}
...
is also used internally by \texttt{mxviewtest} (section~\ref{s:frontends}) to
...
acceleration information, and migration guides from \MCX~2.x to 3.x.


%-------------------------------------------------------------------------------
\subsection{Self-testing the installation (mxtest, mxviewtest)}
\label{s:mxtest}
\indexMCTOOL{mxtest}{textbf}
\indexMCTOOL{mxviewtest}{textbf}

\MCX ships with a Python self-test/benchmark harness, replacing the earlier
\verb+mxrun --test+ command. \verb+mxtest+ compiles and runs a selection (or
...
\index{Data formats}
The \MCX original format, sometimes still referred to by its historical name
``PGPLOT format'' (the plotting package used in \MCX~1.x/2.x), is simply
...
To enable parallel computation, compile \texttt{mcxtrace}-generated C code
...
The efficiency of \MCX using MPI has been tested on large clusters, up to 500
