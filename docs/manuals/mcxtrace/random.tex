...
\index{Monte Carlo method!Random number, KISS}
\index{Monte Carlo method!Random number, Mersenne Twister}
As of \MCX\ 3.x, the default random number generator is a 64-bit
...
Takuji Nishimura\cite{matsumoto1998mersenne} (see \\
...
with \verb+-DRNG_ALG=1+ (e.g. via the \verb+MCXTRACE_CFLAGS+
environment variable, or the C-compiler flag options). It has an
extremely large period ($2^{19937}-1$) and negligible
cross-correlations up to 623 dimensions, but should not be selected for
GPU/OpenACC runs.
