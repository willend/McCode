...
Generates LaTeX longtable(s) of the mcrun/mxrun command-line options,
...
Note: tools/Python/mcrun/mcrun.py is shared between McStas and McXtrace
(the "mcrun"/"mxrun" wrapper scripts both invoke the same underlying
Python source, configured for the active flavour via mccode_config), so
this script -- and its generated table/label names -- are identical
regardless of which manual it is built for.

Usage:
    python3 gen_mcrun_options_table.py ../../../tools/Python/mcrun/mcrun.py \
        > mcrun_options_table.tex

Intended to be called from the doc build (see CMakeLists.txt in this
directory) each time the manual is rebuilt, so the option list can never
silently go stale again. The output file mcrun_options_table.tex is
\\input from running.tex.
...
if __name__ == '__main__':
    ...
    out = []
    out.append('%% AUTO-GENERATED by doc/manuals/mcxtrace/gen_mcrun_options_table.py')
    ...
    out.append(fmt_table(mcrun_opts, 'f:mcrunoptions',
        r'\texttt{mxrun}-specific options (compilation, scanning, optimisation, MPI/OpenACC).'))
    out.append('')
